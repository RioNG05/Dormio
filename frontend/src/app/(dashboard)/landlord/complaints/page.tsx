...
import { 
  AlertOctagon, Plus, Search, ChevronDown, Building2
} from "lucide-react";
  ...
  const grievances = [
    {
      id: "GRV-102",
      room: "102",
      tenant: locale === "en" ? "Nguyen Van A" : "Nguyễn Văn A",
      category: t("categoryRepair"),
      title: locale === "en" ? "Bathroom water tap leaking" : "Vòi nước phòng tắm bị rò rỉ nước",
      priority: locale === "en" ? "High" : "Cao",
      status: t("filterProcessing"),
      statusKey: "processing",
      date: "25/08/2026",
      assignee: locale === "en" ? "Nguyen Van Bao (Tech)" : "Nguyễn Văn Bảo"
    },
    {
      id: "GRV-205",
      room: "205",
      tenant: locale === "en" ? "Tran Thi B" : "Trần Thị B",
      category: t("categoryNoise"),
      title: locale === "en" ? "Neighbor room 206 playing loud music after 23:00" : "Phòng bên cạnh 206 gây ồn ào sau 23h",
      priority: locale === "en" ? "Medium" : "Trung bình",
      status: t("filterPending"),
      statusKey: "pending",
      date: "26/08/2026",
      assignee: locale === "en" ? "Unassigned" : "Chưa phân công"
    },
    {
      id: "GRV-301",
      room: "301",
      tenant: locale === "en" ? "Le Van C" : "Lê Văn C",
      category: t("categoryRepair"),
      title: locale === "en" ? "Air conditioner making loud noise and not cooling" : "Máy lạnh phát ra tiếng kêu to và không mát",
      priority: locale === "en" ? "High" : "Cao",
      status: t("filterResolved"),
      statusKey: "resolved",
      date: "20/08/2026",
      assignee: locale === "en" ? "HVAC Technician" : "Thợ điện lạnh"
    },
    {
      id: "GRV-105",
      room: "105",
      tenant: locale === "en" ? "Pham Hoang D" : "Phạm Hoàng D",
      category: t("categorySecurity"),
      title: locale === "en" ? "Fingerprint entrance door access issue" : "Quên chìa khóa cổng khóa vân tay",
      priority: locale === "en" ? "Low" : "Thấp",
      status: t("filterResolved"),
      statusKey: "resolved",
      date: "18/08/2026",
      assignee: locale === "en" ? "Guard Cuong" : "Bảo vệ Cường"
    }
  ];

  const filteredGrievances = grievances.filter(g => 
    statusFilter === "" || g.statusKey === statusFilter
  );
    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-12">
      ...
      <div className="flex justify-end items-center gap-2 mb-2">
        <button className="flex items-center gap-2 px-4 py-2.5 text-xs font-bold text-white bg-[#2ac1bc] hover:bg-[#72b3a3] rounded-xl shadow-md shadow-[#2ac1bc]/20 transition-all cursor-pointer">
          <Plus className="w-4 h-4" /> {locale === "en" ? "Report New Issue" : "Ghi nhận sự cố mới"}
        </button>
      </div>
              ...
              <div className="flex items-center gap-3 px-4 py-2.5 bg-white/5 hover:bg-white/10 transition-colors rounded-2xl border border-white/10 backdrop-blur-md min-w-[135px]">
                ...
                <div className="flex flex-col">
                  <span className="text-[9px] uppercase font-bold text-zinc-400 tracking-wider">
                    {locale === "en" ? "Properties" : "Số tòa nhà"}
                  </span>
              ...
              <div className="flex items-center gap-3 px-4 py-2.5 bg-rose-500/10 hover:bg-rose-500/20 transition-colors rounded-2xl border border-rose-500/30 backdrop-blur-md min-w-[145px]">
                ...
                <div className="flex flex-col">
                  <span className="text-[9px] uppercase font-bold text-rose-400 tracking-wider">
                    {locale === "en" ? "Total Grievances" : "Tổng sự cố"}
                  </span>
      ...
      <div className="bg-white border border-zinc-200 rounded-2xl p-4 flex flex-col sm:flex-row justify-between items-center gap-4 shadow-sm">
        <div className="relative w-full sm:w-72">
          ...
          <input
            type="text"
            placeholder={locale === "en" ? "Search issues, room numbers..." : "Tìm sự cố, số phòng..."}
          ...
          <div key={g.id} className="bg-white border border-zinc-200 rounded-3xl p-6 shadow-sm hover:shadow-md transition-all flex flex-col justify-between space-y-4">
            <div className="space-y-3">
              <div className="flex justify-between items-start gap-3">
                <span className="px-3 py-1 bg-zinc-100 text-zinc-800 font-extrabold text-xs rounded-xl border border-zinc-200">
                  {locale === "en" ? `Room ${g.room}` : `Phòng ${g.room}`}
                </span>
                <span className={`px-3 py-1 text-xs font-bold rounded-full border ${
                  g.statusKey === "resolved" ? "bg-[#2ac1bc]/10 text-[#2ac1bc] border-[#2ac1bc]/30" :
                  g.statusKey === "processing" ? "bg-[#FF6B35]/10 text-[#FF6B35] border-[#FF6B35]/30" :
              ...
              <div>
                ...
                <p className="text-xs text-zinc-500 mt-1">
                  {locale === "en" ? "Tenant: " : "Khách thuê: "}
                  <span className="font-bold text-zinc-800">{g.tenant}</span> • {locale === "en" ? "Date: " : "Ngày gửi: "}{g.date}
                </p>
              </div>
            </div>
            ...
            <div className="pt-4 border-t border-zinc-100 flex items-center justify-between">
              <div className="text-xs text-zinc-500">
                {locale === "en" ? "Assignee: " : "Phụ trách: "}
                <span className="font-bold text-zinc-800">{g.assignee}</span>
              </div>
              <button className="px-3.5 py-1.5 bg-zinc-900 hover:bg-zinc-800 text-white rounded-xl text-xs font-bold transition-all cursor-pointer">
                {locale === "en" ? "Update Status" : "Cập nhật xử lý"}
              </button>
            </div>
          </div>
        ))}
