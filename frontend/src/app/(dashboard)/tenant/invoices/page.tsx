...
export default function TenantInvoicesPage() {
  ...
  const allInvoices = Array.from({ length: 24 }).map((_, i) => {
    ...
    const dienAmount = 300000 + ((i * 37) % 20) * 10000;
    const nuocAmount = 100000 + ((i * 19) % 10) * 10000;
  ...
  const chartData = allInvoices
    .slice(0, 12)
    .reverse()
    .map((inv) => ({
      name: inv.period,
      [t("roomRent")]: inv.details.find((d) => d.name === t("roomRent"))?.value || 0,
      [t("electricityFee")]: inv.details.find((d) => d.name === t("electricityFee"))?.value || 0,
      [t("waterFee")]: inv.details.find((d) => d.name === t("waterFee"))?.value || 0,
      [t("otherServices")]: inv.details.find((d) => d.name === t("otherServices"))?.value || 0,
    }));
        ...
        <div className="w-full md:w-auto shrink-0 rounded-2xl border border-amber-200 bg-gradient-to-br from-amber-50 to-orange-50 p-4 shadow-sm flex items-center gap-6">
          <div>
            ...
            <div className="flex items-end gap-3 mt-1">
              ...
              {isUp ? (
                <div
                  className="flex items-center gap-0.5 text-xs font-bold text-rose-500 mb-0.5"
                  title={locale === "en" ? "Increased from last month" : "Tăng so với tháng trước"}
                >
                ...
                <div
                  className="flex items-center gap-0.5 text-xs font-bold text-emerald-500 mb-0.5"
                  title={locale === "en" ? "Decreased from last month" : "Giảm so với tháng trước"}
                >
            ...
            <p className="text-[11px] font-semibold text-amber-700/80 mt-1.5">
              {locale === "en"
                ? `Due: ${currentInvoice.dueDate} (3 days remaining)`
      ...
      <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm flex flex-col">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
          <h2 className="text-base font-bold text-zinc-900 flex items-center gap-2">
            <CreditCard className="w-5 h-5 text-[#2AC1BC]" />
            {locale === "en"
              ? "Expense Trends (Past 12 Months)"
              : "Thống kê chi phí (12 tháng gần nhất)"}
          </h2>
          ...
          <div className="flex flex-wrap items-center gap-4">
            <label className="flex items-center gap-2 cursor-pointer text-sm font-semibold text-zinc-600 hover:text-zinc-900 select-none">
              <input
                ...
                onChange={() =>
                  setVisibleLines((prev) => ({ ...prev, tienPhong: !prev.tienPhong }))
                }
            ...
            <label className="flex items-center gap-2 cursor-pointer text-sm font-semibold text-zinc-600 hover:text-zinc-900 select-none">
              <input
                ...
                onChange={() =>
                  setVisibleLines((prev) => ({ ...prev, tienDien: !prev.tienDien }))
                }
            ...
            <label className="flex items-center gap-2 cursor-pointer text-sm font-semibold text-zinc-600 hover:text-zinc-900 select-none">
              <input
                ...
                onChange={() =>
                  setVisibleLines((prev) => ({ ...prev, tienNuoc: !prev.tienNuoc }))
                }
      ...
      <div className="rounded-2xl border border-zinc-200 bg-white shadow-sm flex flex-col">
        <div className="p-5 border-b border-zinc-100 flex flex-col sm:flex-row sm:items-center justify-between bg-zinc-50/50 gap-4 rounded-t-2xl">
          <h2 className="text-base font-bold text-zinc-900">
            {locale === "en" ? "Invoice History" : "Lịch sử hoá đơn"}
          </h2>
          <div className="relative">
            ...
            <input
              type="text"
              placeholder={
                locale === "en" ? "Search billing period..." : "Tìm kiếm kỳ thanh toán..."
              }
              value={filterMonth}
              onChange={(e) => setFilterMonth(e.target.value)}
              className="pl-9 pr-4 h-9 rounded-lg border border-zinc-200 text-sm focus:border-[#2AC1BC] focus:ring-1 focus:ring-[#2AC1BC] outline-none w-full sm:w-64"
            />
          </div>
        </div>
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead>
              <tr className="bg-zinc-50 border-b border-zinc-100 text-zinc-500 font-bold uppercase tracking-wider text-xs">
                <th className="px-6 py-4">{locale === "en" ? "Invoice ID" : "Mã hoá đơn"}</th>
                <th className="px-6 py-4">{t("billPeriod")}</th>
                <th className="px-6 py-4">{t("totalAmount")}</th>
                <th className="px-6 py-4">{t("paymentStatus")}</th>
                <th className="px-6 py-4 text-right">{locale === "en" ? "Detail" : "Chi tiết"}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-zinc-100">
                ...
                <React.Fragment key={inv.id}>
                  <tr
                    ...
                    className={`hover:bg-zinc-50 transition-colors ${
                      expandedRows[inv.id] ? "bg-zinc-50/50" : ""
                    }`}
                  >
                    ...
                    <td className="px-6 py-4 font-bold text-zinc-700">
                      {locale === "en" ? `Month ${inv.period}` : `Tháng ${inv.period}`}
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-right">
                      <Button
                        ...
                        className="rounded-lg font-semibold bg-white border border-zinc-200 shadow-xs cursor-pointer"
                      >
                        {expandedRows[inv.id]
                          ? locale === "en"
                            ? "Collapse"
                      ...
                      <td colSpan={5} className="px-6 py-6">
                        <div className="bg-white border border-zinc-200 rounded-xl p-5 shadow-sm max-w-2xl mx-auto">
                          <h4 className="font-bold text-zinc-900 mb-4 text-center">
                            {locale === "en"
                              ? `Fee Breakdown - ${inv.id}`
                              : `Bảng kê chi tiết phí - ${inv.id}`}
                          </h4>
                          <div className="flex flex-col gap-3">
                            {inv.details.map((detail, idx) => (
                              <div
                                key={idx}
                                className="flex items-center justify-between border-b border-dashed border-zinc-200 pb-3 last:border-0 last:pb-0"
                              >
                            ...
                            <div className="flex items-center justify-between pt-3 mt-1 border-t-2 border-zinc-200">
                              <span className="text-sm font-extrabold text-zinc-900 uppercase">
                                {locale === "en" ? "Total" : "Tổng cộng"}
                              </span>
                              <span className="text-lg font-extrabold text-[#2AC1BC]">
                                {formatCurrency(inv.amount)}
                              </span>
                            </div>
                          </div>
                          {inv.status === "unpaid" && (
                            <div className="mt-6 flex justify-center">
                              <Button className="w-full sm:w-auto bg-[#2AC1BC] hover:bg-[#23a8a3] text-white shadow-sm h-10 rounded-lg px-8 font-bold cursor-pointer">
                                {locale === "en"
                                  ? "Pay this invoice"
              ...
              {paginatedInvoices.length === 0 && (
                <tr>
                  ...
                  >
                    {locale === "en"
                      ? `No invoices found matching "${filterMonth}"`
