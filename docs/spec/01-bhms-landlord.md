...
2. **Find or create the tenant:**
    ...
    - **Not found** → landlord enters phone + name → call the shared `UserService.findOrCreateByPhone()` helper (see `07-auth-and-roles.md`, UC-AUTH-03 — **this exact helper is also used by UC-L-19 staff onboarding**, do not duplicate the logic): generates a random password, creates `User(role='poster', mustChangePassword=true, ...)`. The tenant's first login triggers the OTP verification flow described in UC-AUTH-03 — this UC does not need to handle that itself, just call the helper and move on.
